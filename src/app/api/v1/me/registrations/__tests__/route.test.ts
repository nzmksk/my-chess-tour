...
function makeRequest(url = "http://localhost/api/v1/me/registrations") {
...
describe("GET /api/v1/me/registrations", () => {
  ...
  describe("authentication", () => {
    ...
    it("queries registrations for the authenticated user only", async () => {
      ...
      expect(eqMock).toHaveBeenCalledWith(
        "user_id",
        appIdFor(AUTHENTICATED_USER.id),
      );
    });
  });
  ...
  describe("status filter", () => {
    it("applies status filter when provided", async () => {
      ...
      const req = makeRequest(
        "http://localhost/api/v1/me/registrations?status=confirmed",
      );
    ...
    it("accepts comma-separated status values", async () => {
      setQueryResult([]);
      const req = makeRequest(
        "http://localhost/api/v1/me/registrations?status=confirmed,pending_payment",
      );
    ...
    it("returns 400 for invalid status value", async () => {
      const req = makeRequest(
        "http://localhost/api/v1/me/registrations?status=invalid_status",
      );
  ...
  describe("sorting", () => {
    ...
    it("returns 400 for invalid sort field", async () => {
      const req = makeRequest(
        "http://localhost/api/v1/me/registrations?sort=invalid_field",
      );
    ...
    it("returns 400 for invalid order value", async () => {
      const req = makeRequest(
        "http://localhost/api/v1/me/registrations?order=sideways",
      );
