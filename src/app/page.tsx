import { Suspense } from "react";
import WaitlistForm from "@/components/WaitlistForm";
import BuildProgress from "@/components/BuildProgress";
import ProgressSkeleton from "@/components/ProgressSkeleton";

export default function LandingPage() {
  return (
    <main className="flex h-screen flex-col items-center justify-between overflow-hidden px-6 py-10 bg-bg-base">
      {/* Top spacer */}
      <div />

      {/* Centre content */}
      <section className="flex flex-col items-center gap-8 text-center">
        {/* Brand wordmark */}
        <h1 className="text-3xl tracking-wide uppercase text-gold-bright font-cinzel">
          MY Chess Tour
        </h1>

        {/* Divider */}
        <div className="h-px w-16 bg-gold-bright" />

        {/* Headline */}
        <h1 className="text-5xl font-semibold tracking-widest sm:text-6xl text-text-primary font-cinzel">
          Coming Soon
          <span className="text-gold-bright"> 2026</span>
        </h1>

        {/* Sub-copy */}
        <p className="max-w-sm text-base font-light leading-7 text-text-secondary">
          Malaysia&apos;s premier competitive chess circuit is on its way. Be
          the first to know when we launch.
        </p>

        {/* Waitlist form */}
        <WaitlistForm />
      </section>

      {/* Progress bar streams in when GitHub API responds */}
      <Suspense fallback={<ProgressSkeleton />}>
        <BuildProgress />
      </Suspense>
    </main>
  );
}
