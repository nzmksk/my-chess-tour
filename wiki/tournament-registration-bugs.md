...
Update (testing): The webhook (src/app/api/v1/webhooks/chip/route.ts) and migration 008 (settle_registration_payment) are now in place, but success/failure callbacks still don't update the DB in local/staging testing because NEXT_PUBLIC_SITE_URL is http://localhost:3000 (in .env.local and .env.staging) — CHIP cannot reach localhost, so success_callback never fires. This is the cause of the "successful payment not recorded" and "failed payment stays pending" symptoms seen during testing. Verify on a publicly reachable URL (tunnel/staging with the real domain). Separately, failure events require a CHIP dashboard webhook subscribed to purchase.payment_failure pointed at /api/v1/webhooks/chip; success_callback fires on success only. Also fix .env.staging NEXT_PUBLIC_SITE_URL (currently localhost).
