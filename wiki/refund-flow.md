...
- **Sync vs webhook**: a terminal `refunded` response settles immediately (the later `payment.refunded` webhook is a no-op); a `pending_refund` response only records `chip_refund_id`, and the webhook settles.
...
2. **Synchronous `POST /refund/` response shape.** We trust `id` (the refund Payment id, our idempotency key) and `status` to distinguish terminal success from `pending_refund`. If `status` proves unreliable, treat every sync response as pending and rely solely on the webhook (`chipRefundOutcome` already defaults unknown → `pending`).
