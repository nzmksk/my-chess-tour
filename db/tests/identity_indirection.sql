...
BEGIN
  ...
  -- ===== I6: an unlinked session resolves to NULL and sees nothing =========
  PERFORM set_config('request.jwt.claims', json_build_object('sub', gen_random_uuid())::text, true);
  IF app_user_id() IS NOT NULL THEN
    RAISE EXCEPTION 'I6 FAIL: app_user_id() returned non-NULL for an unlinked auth id';
  END IF;
  SELECT count(*) INTO v_count FROM registrations;
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'I6 FAIL: an unlinked session read % registrations (must fail closed)', v_count;
  END IF;
  RAISE NOTICE 'I6 PASS — unlinked session resolves NULL and every policy fails closed';
